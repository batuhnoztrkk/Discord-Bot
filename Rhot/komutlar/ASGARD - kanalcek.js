...
exports.run = async(client, message, args, ayar, emoji) => {
  ...
  if (message.member.hasPermission("ADMINISTRATOR")) {
    ...
    message.channel.send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).setDescription(`${uye}, ${message.author} Seni Ses Kanalına Çekmek İstiyor, Kabul Ediyormusun?`)).then(async msj => {
      await msj.react('✅');
	...
	if (cevap) {
		...
		.setColor('BLACK')
		.setTitle("KANAL ÇEKME")
		.setThumbnail(message.author.avatarURL())
		.setDescription(`**• Kanal çekme isteği atan kullanıcı : ${message.member} \n• Çektiği kullanıcı : ${uye} \n• Çektiği kanal : <#${message.member.voice.channelID}>**`)
		message.client.channels.cache.get('835643762234753114').send(unban);
